...
class AudioDevice:
	...
	def registerHandler(self, handler):
		...
		while not self.sound:
			try:
				self.sound = Sine(params['sndFrequency'], params['sndDuration'])
			except:
				pass
	...
	def _handleOneTrial(self):
		...
		for i in range(0, self.paramsSignalsCnt):
			self.playSignal()
			if(i < (self.paramsSignalsCnt-1)):
				tc.sleep(self.paramsSignalCycle)

	def handleSignalQueued(self):
		debug("PLAYING SIGNAL", 1)
