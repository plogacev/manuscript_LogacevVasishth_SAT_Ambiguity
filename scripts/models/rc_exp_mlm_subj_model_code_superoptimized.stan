...
functions {
    ...
    real transform_critinit(real x) {
        return 10 * (inv_logit(x) - .5);
    }
    real transform_critasymptote(real x, real init) {
        return 10 * (inv_logit(x) - .5) - init;
    }
    real transform_critinvrate(real x) {
        return exp(x) + 10;
    }

    real transform_asymptote(real x) {
        return 10 * inv_logit(x);
    }
    real transform_invrate(real x) {
        return exp(x) + 10;
    }

    real dprime_fn(real time, real disabled, real asymptote, real invrate, real intercept)
    {
    ...
    real criterion_fn(real time, real init, real asymptote, real invrate, real intercept)
    {
      real criterion; 
      criterion = init;
...
transformed parameters { 
  ...
  for (n in 1:N) { 
    critintercept = eta_critintercept[n];
    intercept = eta_intercept[n];

    if (n == 1 || eta_asymptote[n] != eta_asymptote[n-1] )
      asymptote = transform_asymptote( eta_asymptote[n] );
    if (n == 1 || eta_invrate[n] != eta_invrate[n-1] )
      invrate = transform_invrate( eta_invrate[n] ); 

    if (n == 1 || eta_critintercept[n] != eta_critintercept[n-1] )
      critinit = transform_critinit( eta_critinit[n] );
    if (n == 1 || eta_critasymptote[n] != eta_critasymptote[n-1] )
      critasymptote = transform_critasymptote(eta_critasymptote[n], critinit); 
    if (n == 1 || eta_critinvrate[n] != eta_critinvrate[n-1] )
      critinvrate = transform_critinvrate( eta_critinvrate[n] ); 

    // compute non-linear predictor 
    eta[n] = criterion_fn(C[n, 1] , critinit, critasymptote, critinvrate, critintercept) + 
             dprime_fn(C[n, 1], C[n, 2], asymptote, invrate, intercept); 
  } 
} 
model { 
